...
def left_multiply(state, apply_to: list, op, is_ket=False, pauli=False):
    ...
    if not pauli:
        ...
        for i in range(len(apply_to)):
            if op[i] == 'X':  # Sigma_X
                out = qubit.left_multiply(out, [n * apply_to[i]], ['X'], is_ket=is_ket, pauli=True)
            elif op[i] == 'Y':  # Sigma_Y
                out = qubit.left_multiply(out, [n * apply_to[i], n * apply_to[i] + 1], ['Y', 'Z'], is_ket=is_ket,
                                          pauli=True)
            elif op[i] == 'Z':  # Sigma_Z
                out = qubit.left_multiply(out, [n * apply_to[i], n * apply_to[i] + 1], ['Z', 'Z'], is_ket=is_ket,
                                          pauli=True)
        return out
