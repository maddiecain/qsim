...
class TestQubit(unittest.TestCase):
    def test_single_qubit_multiply_pauli(self):
        ...
        psi0 = qubit.multiply(psi0, [1], 'Y', is_ket=True, pauli=True)
        self.assertTrue(psi0[2 ** (N - 2), 0] == 1j)

        # Apply sigma_z on the second qubit, state is -1j|010000>
        psi0 = qubit.multiply(psi0, [1], 'Z', is_ket=True, pauli=True)
        self.assertTrue(psi0[2 ** (N - 2), 0] == -1j)

        # Apply sigma_x on all qubits but 1
        psi0 = qubit.multiply(psi0, [0, 2, 3, 4, 5], ['X'] * (N - 1), is_ket=True, pauli=True)
        ...
        # Density matrix test
        psi1 = np.array([tools.tensor_product([np.array([1, 1]), np.array([1, 0])]) / 2 ** (1 / 2)]).T.astype(
            np.complex128)
        ...
        psi1 = qubit.multiply(psi1, [0], 'Y', is_ket=False, pauli=True)
    ...
    def test_single_qubit_multiply(self):
        ...
        psi0[0][0] = 1
        # Apply sigma_y on the second qubit to get 1j|010000>
        psi0 = qubit.multiply(psi0, [1], qubit.Y, is_ket=True, pauli=False)
        self.assertTrue(psi0[2 ** (N - 2), 0] == 1j)

        # Apply sigma_z on the second qubit, state is -1j|010000>
        psi0 = qubit.multiply(psi0, [1], qubit.Z, is_ket=True, pauli=False)
        self.assertTrue(psi0[2 ** (N - 2), 0] == -1j)

        # Apply sigma_x on qubits
        psi0 = qubit.multiply(psi0, [0, 2, 3, 4, 5], tools.tensor_product([qubit.X]*(N-1)), is_ket=True, pauli=False)
        ...
        psi0 = qubit.multiply(psi0, [0], qubit.Y, is_ket=False, pauli=False)
        self.assertTrue(np.linalg.norm(psi0 - psi2) <= 1e-10)

        # Test on ket
        psi1 = qubit.multiply(psi1, [0], qubit.Y, is_ket=False, pauli=False)
    ...
    def test_rotation(self):
        ...
        for i in range(N):
            psi0 = qubit.rotation(psi0, [i], np.pi / 4, qubit.Y, is_ket=True, is_involutary=True)
            psi2 = qubit.rotation(psi2, [i], np.pi / 4, qubit.Y, is_ket=True)
        ...
        for i in range(N):
            psi0 = qubit.rotation(psi0, [i], np.pi / 4, ['Z'], is_ket=True, is_involutary=True, pauli=True)
            psi0 = qubit.rotation(psi0, [i], np.pi / 4, qubit.X, is_ket=True, is_involutary=True)
    ...
    def test_multi_qubit_multiply(self):
        ...
        psi0 = qubit.multiply(psi0, [1, 3, 4], op, is_ket=True, pauli=False)
