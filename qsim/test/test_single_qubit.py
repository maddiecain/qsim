...
from qsim import single_qubit

class TestSingleQubit(unittest.TestCase):
    def test_multiply_single_qubit(self):
        N = 6
    ...
    def test_rotate_single_qubit(self):
        N = 6
        # initialize in |000000>
        psi0 = np.zeros(2**N)
        psi0[0] = 1
        ...
        self.assertAlmostEqual(np.vdot(psi1, np.ones(2**N)/2**(N/2)), 1)
        ...
        self.assertTrue(np.abs(np.vdot(psi0, psi1)*np.exp(1j*np.pi/4*N) - 1) <= 1e-10)


    def test_multiply_single_qubit_mixed(self):
        ...
        self.assertTrue(np.linalg.norm(single_qubit.multiply_single_qubit_mixed(
            rho, 0, 2) - rho2) <= 1e-10)


    def test_operate_single_qubit_mixed(self):
        psi = np.kron([1,1], [1,0])/2**(1/2)
        rho = np.outer(psi, psi)
        ...
        self.assertTrue(np.linalg.norm(single_qubit.operate_single_qubit_mixed(
