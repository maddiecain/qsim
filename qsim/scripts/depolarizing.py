...
for Ep in np.linspace(1, 100, 10):
    ...
    m_error = master_equation.MasterEquation(h, noise_models.DepolarizingNoise(.05))
    res_error = m_error.run_ode_solver(s.state, 0, 1)
    fidelity = np.trace(np.transpose(res.conj(), axes = [0, 2, 1])@res_error, axis1=1, axis2=2)

    plt.title('|+> in JFS code under energy penalty and depolarizing noise')
    plt.xlabel('Time')
    plt.ylabel('Fidelity with noiseless state with noise probability 0.05')
    print(str(Ep))
